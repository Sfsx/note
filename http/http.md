...
+ 子群的阶数 $n$（$n$ 为 $G(x, y)$ 的阶）

从 $\{1,...,n-1\}$ 中随机选择一个整数 $Xb$ 作为私钥（$n$ 为子群的阶数）。ab 交换 $Pa, Pb$ 后计算出共同的密钥 $Sa$。由于已知 $Pb, G$ 反求 $Xb$ 是求解椭圆曲线离散对数问题（ECDLP），这是个非常困难的问题，从而保证了私钥的有效性。
...
+ 一个用于生成循环子群的基点 $G(x, y)$ （椭圆线上任意点）
+ 子群的阶数 $n$（$n$ 为 $G(x, y)$ 的阶）
+ 产生一个随机整数 $d_A\ (d_A < n)$，作为私钥;
+ 公钥为 $Q_A = d_A \times G$

签名：

1. 假设要签名的原始数据为 m，计算 $e = HASH(m)$ 计算 z
2. 从 $[1, n-1]$内，随即选择一个整数 k
3. 计算 $P(x_p, y_p) = kG$
4. 计算 $r = x_p\ mod\ n$，如果 $r=0$ 返回步骤3重新计算
5. 计算 $s=k^{-1}(z + rd_A)\ mod\ n$，如果 $s=0$ 返回步骤3重新计算

验证：

1. 假设要签名的原始数据为 m，计算 $e = HASH(m)$ 计算 z
2. 计算 $u_1 = s^{-1} z \ mod\ n$
3. 计算 $u_2 = s^{-1} r \ mod\ n$
4. 计算 $P(x_p, y_p) = u_1 G + u_2 Q_A$

    $$
    \begin{aligned}
    P(x_p, y_p) &= u_1G+u_2Q_A \\
    &= u_1 G + u_2 d_AG \\
    &= (u_1 + u_2 d_A)G \\
    &= (s^{-1} z + s^{-1} r d_A)G \ mod\ n \\
    &= s^{-1}(z + r d_A)G\ mod\ n \\
    \end{aligned}
    $$
    $$
    s = k^{-1}(z + r d_A)\ mod \ n \\
    kG = s^{-1}(z + rda)G\ mod\ n = P(x_p, y_p)
    $$

5. 若 $r \equiv x_p\ mod\ n$ 验证成功
