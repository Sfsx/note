...
1. 全局注册回调函数（例如 `function callback() { ... }`）
2. 通过 script 标签请求跨域接口，并使用回调函数名作为 get 请求参数。
3. 服务器收到请求后，拿到请求参数中的回调函数名，动态生成 js 文件，文件中只需要写入一句调用回调函数的代码，其中调用参数为 web 端所需数据（例如 `callback({ data: 'hello' })`）。将 js 文件作为 get 请求返回
4. web 端接收 js 文件后，会立即执行 js 文件中的代码。此时 web 端的回调函数就会被执行，调用参数即为服务端写入的调用参数（即本次请求所需的数据）。
