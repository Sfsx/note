...
1. 初始化配置参数
   从shell命令中读取的配置，与 config js 文件中读取的配置配合，得到最终参数
2. 开始编译 
   初始化 compiler 对象，注册所有插件，监听 wepback 构建的生命周期事件
3. 确定入口文件 
   通过配置 entry 确定入口模块
4. 编译模块 
   通过配置的 loader 对模块进行编译，为模块添加 module 对象，通过AST解析源码收集依赖关系，对依赖模块递归进行上述操作
5. 完成模块编译 
   根据入口文件以及模块的依赖关系，组装成一个个 chunk，添加到输出列表
6. 输出
   根据配置的输出目录和文件名，把文件写入文件系统