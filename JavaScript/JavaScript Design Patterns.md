...
+ 同步加载
+ 模块内代码只会运行在模块作用域内，不会污染全局作用域
+ 模块可以引入多次，但只会在第一次加载的时候运行一次，后面运行都是取缓存的值。如果要让模块再次运行，必须清除缓存

#### AMD

+ 异步加载
+ 预先加载所有依赖，先执行模块的所有依赖，在执行模块内部代码
+ 推崇依赖前置，定义模块的时候就要声明其依赖

#### CMD

+ 异步加载
+ 预先加载所有依赖，在执行模块的过程中，遇到依赖，才会执行改依赖
+ 推崇依赖就近，只有在用到某个模块的时候再去 require
...
2. CommonJS 模块是运行时加载，ES6 模块是编译时输出接口。
    ...
    + 编译时加载：JS 引擎对脚步静态分析的时候，遇到模块加载命令 `import`，就会生出一个只读引用。等到脚步真正执行时，再根据这个只读引用，到被加载的模块里面去取值。而不是加载整个模块。这种加载称之为“编译时加载”。
